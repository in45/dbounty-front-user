...
contract DbountyContract is AdminContract , HTTToken {
    ...
    event CompanyCreated(address caller,string  _idCompany,uint256 _balance);
    ...
    function createCompany(string  memory _id, uint256 _balanceCompany,address _manager1) public  returns(bool){
        ...
        emit CompanyCreated(msg.sender, _id, _balanceCompany);
         managers[_manager1]="sysalpha";
         companies[_id].managers[_manager1]=true;
        return true;
    }
    function createProgram(string  memory _idCompany,string  memory _id, address _manager) public  returns(bool){
    ...
    function addManager(string  memory _company_id,address _manager_address,string memory role) public {
        
    managers[_manager_address]=role;
    companies[_company_id].managers[_manager_address]=true;
    
    }
    function createReport(string memory _idProgram,string  memory _id, address _user,string memory _content) public returns(bool){
         Report memory report_obj = Report(_id,keccak256(abi.encodePacked(_content)),_user,"new",0,false,address(0),address(0));
         reports[_id]=report_obj;
         programs[_idProgram].reports[_id]=true;
         return true;
    }
     function reportState(string memory _idReport,string  memory _status ,address _manager)  public {
         reports[_idReport].status=_status;
    }
     function approveProgram(string memory _idProgram)  public {
         programs[_idProgram].status="new";
    }
     function approveReport(string memory _idReport,address _manager,uint8 _bounty) public {
         reports[_idReport].status="approved";//logique commit 
         reports[_idReport].bounty_win = _bounty;
    }
    function paid(string memory _idReport,address _manager) public payable  {
